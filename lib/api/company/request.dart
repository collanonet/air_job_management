...
class RequestApiService {
  ...
  Future<List<Request>> getAllHolidayRequestByUsers(String userId) async {
    try {
      var doc = await requestRef.where("userId", isEqualTo: userId).where("is_holiday", isEqualTo: true).get();
      if (doc.docs.isEmpty) {
        return [];
      } else {
  ...
  Future<List<Request>> getTotalHolidayLeaveRequest(String userId, String startDate, String endDate) async {
    try {
      ...
      if (doc.docs.isEmpty) {
        ...
        for (var doc in doc.docs) {
          ...
          if (CommonUtils.isDateInRange(date, start, end)) {
            list.add(Request.fromJson(data));
          }
        }
        return list;
      }
