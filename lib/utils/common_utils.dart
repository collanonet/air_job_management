...
class CommonUtils {
  ...
  static calculateOvertimeInEntry(EntryExitHistory entry, {bool? isOvertime, bool? withInLimit, bool? nonSat}) {
    ...
    double legalHours = convertTimeStringToHours(entry.overtimeWithinLegalLimit!);
    if (legalHours > 0) {
      ///Minus legal, because we set nonSat equal to overtimes so overtimes must be nonSat + legal
      List<int> nonStateMinusLegal = calculateBreakTime(entry.nonStatutoryOvertime, entry.overtimeWithinLegalLimit);
  ...
  static totalWorkingWithBreakTime(List<EntryExitHistory> entryList, List<DateTime> dateTimeList, String name) {
    ...
    for (int i = 0; i < entryList.length; i++) {
      DateTime d = DateToAPIHelper.fromApiToLocal(entryList[i].workDate!);
      if (dateTimeList.contains(d) && entryList[i].myUser!.nameKanJi == name) {
        double breakTime = convertTimeStringToHours("${entryList[i].breakingTimeHour}:${entryList[i].breakingTimeMinute}");
        double workingHours = convertTimeStringToHours("${entryList[i].workingHour}:${entryList[i].workingMinute}");
        double total = breakTime + workingHours;
        var data = workingHours == 0 ? "00:00" : convertToHoursAndMinutes(total);
