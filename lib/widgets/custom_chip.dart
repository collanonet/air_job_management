...
class _CustomChipTagsState extends State<CustomChipTags> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: <Widget>[
        Visibility(visible: widget.chipPosition == ChipPosition.above, child: _chipListPreview()),
        Form(
          key: _formKey,
          child: TextFormField(
            ...
            decoration: widget.decoration ??
                InputDecoration(
                  ...
                  hintText: widget.createTagOnSubmit ? "Submit text to create Tags" : "Separate Tags with '${widget.separator ?? 'space'}'",
                ),
                ...
                : (value) {
                    ...
                    if (_formKey.currentState!.validate()) {
                      if (value.endsWith(widget.separator ?? " ")) {
                        ...
                        if (value != widget.separator && !widget.list.contains(value.trim())) {
                          widget.list.add(value.replaceFirst(widget.separator ?? " ", '').trim());
                        }
        ...
        Visibility(visible: widget.chipPosition == ChipPosition.below, child: _chipListPreview()),
      ],
    );
  }
