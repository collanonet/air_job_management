...
class JobPostingForCompanyProvider with ChangeNotifier {
  ...
  onInitForJobPostingDetail(String? id) async {
    ...
    if (fromSalaryAmount.text.isNotEmpty) {
      fromSalaryAmount.text = formatter
          .format(int.parse(fromSalaryAmount.text))
          .replaceAll(".00", "");
    }
    if (toSalaryAmount.text.isNotEmpty) {
      toSalaryAmount.text = formatter
          .format(int.parse(toSalaryAmount.text))
          .replaceAll(".00", "");
    }
    ...
    employeePromotionAvailable =
        jobPosting?.employeePromotionAvailable ?? false;
    qualificationAcqSupportSystem =
        jobPosting?.qualificationAcqSupportSystem ?? false;
    ...
    dormCompanyHouseHousingAllowanceAvailable =
        jobPosting?.dormCompanyHouseHousingAllowanceAvailable ?? false;
    qualificationAllowance = jobPosting?.qualificationAllowance ?? false;
    perfectAttendanceAllowance =
        jobPosting?.perfectAttendanceAllowance ?? false;
    ...
    minimumNumberOfWorkingDays.text =
        jobPosting?.minimumNumberOfWorkingDays ?? "";
    minimumNumberOfWorkingTime.text =
        jobPosting?.minimumNumberOfWorkingTime ?? "";
    ...
    houseWivesHouseHusbandsWelcome =
        jobPosting?.houseWivesHouseHusbandsWelcome ?? false;
    ...
    individualityCanBeUtilized =
        jobPosting?.individualityCanBeUtilized ?? false;
    standingWork = jobPosting?.standingWork ?? false;
    deskWork = jobPosting?.deskWork ?? false;
    tooMuchInteractionWithCustomers =
        jobPosting?.tooMuchInteractionWithCustomers ?? false;
    lessInteractionWithCustomers =
        jobPosting?.lessInteractionWithCustomers ?? false;
    ...
    noKnowledgeOrExperienceRequired =
        jobPosting?.noKnowledgeOrExperienceRequired ?? false;
