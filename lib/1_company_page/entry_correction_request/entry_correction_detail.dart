...
class _EntryCorrectionRequestDetailPageState extends State<EntryCorrectionRequestDetailPage> with AfterBuildMixin {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                "交通費（${entryExitHistory?.transportationFeeOptions ?? ""}）",
                style: kNormalText,
              ),
              Text(
  ...
  getData() async {
    ...
    var afterCutOvertimeForUpdate = calculateBreakTime(
        "${actualWorkingDataForUpdate[0]}:${actualWorkingDataForUpdate[1]}", entryExitHistory!.entryExitHistoryCorrection!.overtime);
    actualWorkingHoursForUpdate = convertTimeStringToHours("${afterCutOvertimeForUpdate[0]}:${afterCutOvertimeForUpdate[1]}");
